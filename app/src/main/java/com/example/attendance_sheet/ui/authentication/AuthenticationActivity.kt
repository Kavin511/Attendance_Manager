package com.example.attendance_sheet.ui.authentication
...
import com.example.attendance_sheet.databinding.ActivityAuthenticationBinding
import com.example.attendance_sheet.db
import com.example.attendance_sheet.viewModels.AuthenticationViewModel
...
class AuthenticationActivity : AppCompatActivity() {
    ...
    private fun initialiseDb() {
        Realm.init(this)
        val config = RealmConfiguration.Builder().name(db).schemaVersion(1).deleteRealmIfMigrationNeeded().allowWritesOnUiThread(false).build()
    ...
    private fun initialiseTabs() {
        ...
        TabLayoutMediator(tabLayout,viewPager){
            tab,position -> tab.text=auth[position]
        }.attach()
